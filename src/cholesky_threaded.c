    ...
    )
{
  ...
  me = workspace;
  ma = workspace + thread_id * 4 * block_size * block_size + 2 * block_size *
block_size; mb = ma + block_size*block_size; mc = mb + block_size*block_size; md
...
mc);
      ...
      if (inverse_upper_triangle_block_and_diagonal(pij_n, mb, diagonal+i, me +
block_size*block_size * ((i/block_size)%2))) // me is inversed diagonal block
        return -1;
    }
...
mc);
      ...
      main_blocks_multiply(pij_n, pij_n, pij_m, md, mb, mc); // md is inversed
diagonal block cpy_block_to_matrix_block(matrix, i, j, matrix_size, pij_n,
pij_m, mc);
    }
  }
